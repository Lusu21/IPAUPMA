...
try {
    ...
    $sector             = trim(mb_strtoupper($_POST['sector'] ?? ''));
    $apellido           = trim(mb_strtoupper($_POST['apellido'] ?? ''));
    $cedula             = trim($_POST['cedula'] ?? '');
    $rif                = trim(mb_strtoupper($_POST['rif'] ?? ''));
    $fecha_nacimiento   = $_POST['fecha_nacimiento'] ?? null;
    $grado_instruccion  = $_POST['grado_instruccion'] ?? null;
    $oficio_ocupacion   = trim(mb_strtoupper($_POST['oficio_ocupacion'] ?? ''));
    ...
    $tipo_cultivo           = trim(mb_strtoupper($_POST['tipo_cultivo'] ?? ''));
    $area_cultivada         = trim(mb_strtoupper($_POST['area_cultivada'] ?? ''));
    $tiempo_sembrado        = trim(mb_strtoupper($_POST['tiempo_sembrado'] ?? ''));
   ...
   $sql_agricultura = "INSERT INTO agricultura (
    ...
    if (isset($_POST['fam_nombre']) && is_array($_POST['fam_nombre'])) {
        ...
        foreach ($nombres as $index => $fam_nombre) {
            $fam_nombre_val    = trim(mb_strtoupper($fam_nombre));
            $fam_edad_val      = intval($edades[$index] ?? 0);
            $fam_cedula_val    = trim($cedulas[$index] ?? '');
            $fam_parentesco_val= trim(mb_strtoupper($parentescos[$index] ?? ''));
            $fam_ocupacion_val = trim(mb_strtoupper($ocupaciones[$index] ?? ''));
